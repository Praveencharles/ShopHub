...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # If the catalog already contains any data, skip the seed.
        if (
            ...
            self.stdout.write(
                self.style.WARNING(
                    "Catalog already contains data. Skipping seed."
                )
            )
            return
        ...
        for cat_name, config in self.CATALOG.items():
            category = Category.objects.create(
                ...
                description=(
                    f"Premium {cat_name.lower()} collection "
                    f"curated for the best quality and value."
                ),
                is_active=True,
            )
            category_count += 1
            ...
            for brand_name in config["brands"]:
                brand = Brand.objects.create(
                    ...
                    description=(
                        f"{brand_name} - trusted "
                        f"{cat_name.lower()} brand."
                    ),
                    is_active=True,
                )
                brand_count += 1

                for idx, product_name in enumerate(
                    config["products"],
                    start=1,
                ):
                    price = Decimal(
                        rng.randint(min_price, max_price)
                    )
                    ...
                    Product.objects.create(
                        ...
                        slug=slugify(
                            f"{brand_name} {product_name} {idx}"
                        ),
                        description=(
                            f"The {brand_name} {product_name} "
                            f"is crafted with premium materials "
                            f"and built to last. Experience the best "
                            f"of {category.name.lower()} with this "
                            f"top-rated product loved by thousands "
                            f"of customers."
                        ),
                        short_description=(
                            f"{brand_name} {product_name} - "
                            f"{category.name} essentials."
                        ),
                        price=price,
                        ...
                        brand=brand,
                        sku=(
                            f"{slugify(cat_name)[:3].upper()}-"
                            f"{slugify(brand_name)[:3].upper()}-"
                            f"{idx:03d}"
                        ),
                        ...
                        dimensions=(
                            f"{rng.randint(5, 60)} x "
                            f"{rng.randint(5, 60)} x "
                            f"{rng.randint(2, 30)} cm"
                        ),
                        tags=(
                            f"{slugify(cat_name)},"
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Seed complete: {category_count} categories, "
